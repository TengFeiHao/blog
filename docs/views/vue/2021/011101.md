...
[代码示例](https://github.com/TengFeiHao/vue-example/blob/master/src/router/index.js)

## vueRouter 源码的实现

单页面应用程序中，url发生变化，不刷新显示对应视图的内容

- SPA 两种实现方式
  - hash  hashChange事件
  - history api  popChange事件

- 事件hashChange，通知router-view更新？
  - 利用vue数据响应式（从而实现url变化render函数重新执行 ）
  - 制造一个响应式数据表示当前url，在router-view的render函数中使用它

## 自己实现一个vueRouter

[代码示例](https://github.com/TengFeiHao/vue-example/blob/master/src/hrouter/hvue-router.js)
...
[代码地址](https://github.com/TengFeiHao/vue-example/blob/master/src/HStore/HVuex.js)
